...
def recognize_face(images, args):
    np.random.seed(seed=args.seed)
    g = tf.Graph()
    with g.as_default():
        # Placeholder for input images
        images_placeholder = tf.placeholder(tf.float32, shape=(1, args.image_size, args.image_size, 3), name='input')

        # Build the inference graph
        prelogits, _ = network.inference(images_placeholder, 1.0, phase_train=False, weight_decay=0.0)
        ...
        train_set = facenet.get_dataset('../datasets/lfw_mtcnnpy_160')
        nrof_classes = len(train_set)
        print('Total number of classes: %d' % nrof_classes)

        with tf.sg_context(name='classifier'):
            logits = prelogits.sg_dense(dim=nrof_classes, bias=True, act='softmax')
        ...
        with sess.as_default():
            ...
            max_ind, coin = sess.run([max_index, confidence], feed_dict=feed_dict)
            # for faceIdex in xrange(len(images)):
            #     print('Result: %s' % train_set[max_ind].name)
            #     print('Score: %f' % coin)
